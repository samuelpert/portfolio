...
const Hero = ({ startAnimation }: { startAnimation: boolean }) => {
  // Use refs to control animation rendering
  const animationCompleted = useRef(false);

  // Mark animation as completed after a certain time
  useEffect(() => {
    if (startAnimation && !animationCompleted.current) {
      const timer = setTimeout(() => {
        animationCompleted.current = true;
      }, 3000); // Time after which animations are considered "complete"
  ...
  return (
    <div className="pb-20 pt-36">
      <div>
        {/* Only apply the spotlight animation if it hasn't completed yet */}
        <Spotlight
          className="-top-40 -left-10 md:-left-32 md:-top-20 h-screen"
          fill="white"
          startAnimation={startAnimation}
          reducedMotion={animationCompleted.current}
        />
        <Spotlight
        ...
        <div className="max-w-[89vw] md:max-w-2xl lg:max-w-[60vw] flex flex-col items-center justify-center">
          <div>
            <img
              src="/me.png"
              alt="Samuel"
              className="rounded-full border-2 border-[#FF751B] w-32 h-32 mb-4"
